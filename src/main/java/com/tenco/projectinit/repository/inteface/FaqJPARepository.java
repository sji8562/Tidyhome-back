...
public interface FaqJPARepository extends JpaRepository<Faq, Integer> {
    @Query(value = "SELECT NEW com.tenco.projectinit.dto.mng.FaqResponseDTO$FaqListDTO(fp.id as id, f.name as name, fp.title as title, fp.code as code) " +
            "FROM Faq f LEFT JOIN FaqPart fp ON f.id = fp.faq.id")
